# Virtual Environment Manager (ve) Shell Integration for Bash
    ...
    if [[ "$cmd" == "activate" && $# -ge 1 ]]; then
        ...
        if [[ $ve_exit_code -eq 0 ]]; then
            ...
            if [[ -n "$activate_script" && -f "$activate_script" ]]; then
                ...
                echo "✅ Environment activated: $(basename "$(dirname "$(dirname "$activate_script")")")"
            fi
        fi
...
# Set up directory change hook for bash
if [[ -n "$BASH_VERSION" ]]; then
    # Override cd to call our function
    _ve_original_cd=$(type -p cd)
    cd() {
        builtin cd "$@" && _ve_auto_activate
    }
